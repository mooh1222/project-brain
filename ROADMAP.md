...
근거 위치 정보를 원래 설계 의도(정본 §6.2)로 복원. 코드 책갈피(EvidenceRef)의 `locator`를
좌표 복사가 아니라 짝 코드 위치 장부(CodeLocator)를 id로 가리키는 **번호표**
(`{code_locator_id}`)로 생성. 엔진이 어디서도 안 읽고 갱신도 안 하던 줄번호
(`line_start`/`line_end`)를 적재·저장에서 제거(스키마는 줄번호 optional 유지 — 거부 규칙
추가가 아니라 `build`가 안 만들 뿐). 회상으로 찾은 객체를 펼쳐보는
...
5. **locator 위치 갱신 / 기존 데이터 정비 (Part B)**
   ...
   - **남은 Part B = 기존 코퍼스 일괄 변경(미룸)**: 옛 데이터의 좌표 복사 locator(문자열·객체)와
     evref에 남은 줄번호를 번호표로 통일하는 마이그레이션. **실익 0**(그 칸은 색인·회상·랭킹·

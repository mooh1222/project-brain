...
### 반복 색인의 자동 벡터 재사용 (#57·#58)

기존 `index rebuild`에서 마지막 정상 색인의 exact surface 벡터를 자동 재사용하고,
호환되지 않거나 불완전한 색인은 전체 계산으로 전환하는 구현을 추가했다. 새 DB 전체 조립과
원자 교체는 유지한다. 실모델 배치 의존성을 제거한 이유와 cold/warm 비교는
[검증 보고서](docs/reports/issue-58-vector-reuse.md)에 기록한다.
BB2에서 8,044개 전량 재사용·새 계산 0건, cold/warm 색인·검색 결과 동치와 eval 18/18을
확인했다. BB2 별도 작업으로 인한 기존 raw 청크 수 가드 불일치는 사용자가 이번 기능의
완료 차단 사유에서 제외하기로 확인했다.

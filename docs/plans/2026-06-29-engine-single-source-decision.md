# 엔진 단일원본(스킬 관리) — 결정·구현 완료 (역사 기록)

> **상태(2026-06-29):** 1번(installer 디렉토리 통째 주입)으로 **결정·구현·main 머지·origin 푸시
> 완료**(`6d6a936`..`c9eda64`), 합성 537 통과, bb2 채택 19파일 diff0. footgun(config 누락 키
> backfill)도 `c9eda64`로 닫힘. 아래 본문은 **결정 시점(구현 전, 커밋 `edc2f88`) 기록**이며,
> 미결·착수 전·"현재 코드 사실" 블록은 모두 당시 기준이다(현재 상태와 다름 — 각 블록 머리의
> 라벨 참조).
>
> **목적(구현 전 기록):** 우선순위4("엔진이 스킬의 단일 원본")의 **배포모델 결정**을 다음 세션이 바로
...
## 결정할 것 — 딱 하나 (결정 시점 기록 — 1번으로 확정·구현됨)

> 아래는 결정 직전 기록이다. **결과: 1번(installer 디렉토리 통째 주입)으로 확정해 구현·머지 완료.**
...
## 현재 코드 사실 (구현 전 진단 — 아래는 당시 문제 상태이고 모두 해소됨)

> **주의:** 이 블록은 제목과 달리 "현재 사실"이 아니라 **구현 직전 진단**이다. 아래 모든 항목은
> 1번 구현으로 정반대가 됐다. 미래 세션은 이 블록을 현재 코드 상태로 읽지 말 것.
...
- ~~**installer는 `SKILL.md`만 주입한다.**~~ `install()`(`src/project_brain/installer.py`)이
  ...
  **→ 해소:** install이 `templates/<skill>/` 디렉토리를 통째 `rglob` walk 해 SKILL.md +
  references/ + scripts/ 전부를 주입한다. 상수명도 `_SKILLS`(query/ingest/session-ingest/audit 4종),
  대상 경로는 `.agents/skills/<project>-<suffix>/`(`6d6a936`).
- ~~**bb2 manifest = `{"files": {}}` (빔).**~~ install은 manifest에 없는 기존 파일을 "사용자 소유"로
  보고 skip → bb2에 install을 돌려도 아무것도 안 바뀐다(전파 0).
  **→ 해소:** bb2에서 install 돌려 **19파일 채택(diff0)** — 디스크==렌더라 안전하게 adopt됐다.
- ~~**레이아웃 불일치.** installer는 `.claude/skills/`에 쓴다고 가정~~하지만, bb2는
  `.agents/skills/bb2-brain-*`가 원본이고 `.claude/skills/*`는 그 심볼릭 미러. 엔진은
  `.agents/skills/` 구조를 모른다.
  **→ 해소:** install이 단일 원본인 `.agents/skills/`에 쓴다.
- ~~**미주입 자산 규모(install 밖에 있는 것):**~~ `bb2-brain-ingest` SKILL.md 외 **23 파일**
  ...
  **→ 해소:** 디렉토리 통째 walk라 references/scripts도 전부 주입 범위 안.
- ~~**엔진 templates 현재 범위:** `query.md / ingest.md / session-ingest.md / audit.md` + `CHANGELOG.md`~~
  (평면 `.md` 파일). references/scripts 하위 디렉터리 자체가 없음(= 베이스가 SKILL.md급만 보유).
  **→ 해소:** templates는 이제 `<skill>/SKILL.md` 디렉토리 구조(+ ingest는 references/·scripts/,
  session-ingest는 references/) — bb2 정합본을 역수입(`5ca5405`)해 채웠다.
- ~~**하드코딩(역수입 시 일반화 대상):**~~ bb2 `scripts/`에 리터럴 `bb2_client` 5파일, `develop` 1파일
  ...
  **→ 해소:** render_text가 `{{PROJECT}}`·`{{BRAIN_ROOT}}`·`{{DEFAULT_BRANCH}}`·`{{REPO}}` 4변수만
  최소 치환(`8a896d6`). 소비처가 bb2 하나뿐이라 도메인 예시는 bb2색 리터럴 유지(범용화 아님).
...
## 착수 위험 — 순서 엄수 (1번 택할 때) — 결정 시점 기록, 이미 실행됨

> 아래 절대순서는 구현 전 계획이며 **그대로 실행돼 완료**됐다. 각 단계에 실제 커밋을 매핑했다.

**역수입(bb2 → 엔진) 먼저 → 엔진 머지 → 그 다음에만 manifest 채우기/install.**
순서를 어기면 엔진의 (짧은) 베이스 템플릿이 bb2의 (풍부한) 스킬을 덮어 발전분이 소실된다.
manifest가 비어 전부 skip이고 `--force`가 없는 (당시) 상태가, 역수입 전 잘못 install해도
덮지 않게 막아주는 안전판이기도 하다(채우는 순간 사라짐 — 그래서 채우기가 맨 마지막).

내부 절대순서(1번) — 괄호 안은 실제 실행 커밋:
1. installer를 디렉토리 walk로 확장(제외 필터 + 치환). templates/에 references/·scripts/ 신설. **(walk=`6d6a936`)**
2. bb2 정합본 역수입: `bb2`→`{{PROJECT}}`, REPO·`develop` 일반화, 도메인 예시 일반화, fixtures 제외. **(역수입=`5ca5405`, 변수 치환=`8a896d6`, synonyms 복원=`6722d65`)**
3. 엔진 머지(이미 정합된 베이스 항목 제외 — 이번 세션이 advisories·UTC Z→KST·graph isolated B+C·줄번호·audit까지 맞춰둠). **(force+adopt=`e7c16d7`, manifest 방어=`03f2780`, 머지=`8f0e79a`)**
4. **역수입 완료 확인 후에만** manifest 채우기/install. → bb2 채택 19파일 diff0로 검증.
5. 이후 규율: "bb2 직접수정 금지"(단일원본의 대가).
6. (추가) config 누락 키 backfill footgun도 닫음 — 기존 키·빈 값 미변경, 값 있는 누락 키만 채움. **(footgun=`c9eda64`)**
...
## 권장 — 결정 시점 기록 (당시 "급하지 않다"였으나 1번을 착수·완료함)

> 아래는 결정 직전의 "급하지 않으니 착수 신호를 기다리자"는 권고다. **실제로는 이 세션 직후
> 1번을 착수해 위 절대순서대로 구현·머지·푸시까지 완료했다.**

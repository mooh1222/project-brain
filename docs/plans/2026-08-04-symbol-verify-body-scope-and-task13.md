...
**Task 13을 먼저 한다**고 정했다. 근거는 계획서 원문
(`docs/superpowers/plans/2026-07-31-task17-canonical-id-recovery.md`, Task 13 Step 2)이
**스냅샷 안의 저장소 HEAD가 `TASK17_COMMIT`과 정확히 같을 것**을 요구한다는 점이었다.
`--3`을 먼저 고치면 BB2에 새 커밋이 생겨 그 조건이 깨진다고 봤다.

```
1) Task 13  →  2) 엔진 수정  →  3) --3 교정  →  4) 재검증
```

### 실제 순서 (이렇게 했다)

**그 근거가 실행 중에 사라졌다.** BB2 커밋을 만들지 않기로 결정되면서(§7.1 참고)
`--3`을 고쳐도 HEAD가 `e28ff4ee7d`에 그대로 머문다. 즉 Task 13을 먼저 할 이유가 없고,
오히려 나중에 해야 최종 상태를 담는다.

```
1) 엔진 수정 — 몸통 규칙 (TDD)
2) --3 교정 + 색인 재구축
3) 재검증 — audit ok=true
4) Task 13 — 최종 스냅샷 + Task 18 연결점
```

Task 13을 먼저 한 번 찍었는데(`f0a9522a…`), 그건 심볼 수정 전 상태라 최종본이 아니다.
심볼 작업 동안 되돌림 지점으로 쓴 뒤 제거하고, 끝난 상태로 다시 찍었다
(`ad657ec5…`, `.snapshots/2026-08-04/task17-final/`).

아래 §3은 원래 순서대로 Task 13을 먼저 적어두었다. 절차 자체는 그대로 유효하니
내용은 남기되, **실행은 마지막에 했다**는 점을 여기 적어둔다.
...
- [x] 엔진 HEAD — 실행 시점 값을 쓴다. 최종 스냅샷은 `148c9e7d676d239b614fd742edfe3a596fa33219`로 찍었다
      (계획 초안의 `3a91f868`은 몸통 규칙 커밋 전 값이라 낡았다)
- [x] 승인 영수증 존재: `task8-ledger-approval.json`,
      `task17-rebound-approval-4e7638afe82c.json`
- [x] ~~엔진 미추적 문서 9개를 먼저 커밋~~ — **철회.** 완료 조건의 "엔진이 깨끗한 커밋
      상태"는 워크트리(`ENGINE`)를 가리키고 그쪽은 이미 깨끗하다. 메인 클론의 미추적
      문서는 보존 대상인 `SOURCE_CHECKOUT` dirt다. 근거는 §7.1.
...
모양이다. 짝인 `evref` 카드의 `title`도 같다.

엔진 쪽은 이미 `a20b1c5`("앵커 제목을 심볼에 맞춘다")로 고쳐졌지만 **기존 데이터는
소급 적용되지 않았다.** 이번에 `--3`을 건드리는 김에 제목도 함께 맞춘다.

**정정 — 무엇으로 바꾸는가.** 처음엔 "사람이 읽을 수 있는 문장으로 바꾼다"고 적었다.
**규약과 반대다.** `assembly.py:65-66`이 명시한다.
...
제목은 **symbol과 같아야 한다.** 읽는 쪽이 코드와 대조해 진위를 확인할 수 있는 값만
쓴다는 뜻이다. 사람이 쓴 설명문은 대조가 불가능해서 금지된다.

그리고 `evref`는 `title`만 고친다. `EvidenceRef.summary`는 인용문을 담는 것이 정상이라
(`assembly.py:79` — `"summary": quote[:500]`) 건드리지 않는다.
...
### 7.2 골든셋 서식 복원 — **하지 않는다** (판단 철회)

처음엔 "`brain/eval_scenarios.json`이 216줄에서 1줄로 눌렸으니 사람이 읽을 수 있게
되돌린다"고 적었다. **두 가지가 틀렸다.**

**틀린 것 1 — 사람이 편집하는 파일이 아니다.** 사용자가 지적했고 이력으로 확인했다.
이 파일의 변경 7회가 **전부 에이전트 작업**이다. 사람이 시나리오를 추가하거나 기대값을
고친 적이 없다.

**틀린 것 2 — 서식을 "잃은" 게 아니라 정규 형식으로 통일된 것이다.** 엔진이 이 파일을
쓸 때 쓰는 함수가 `migration.py:514`다.
...
공백을 없애고 키를 정렬하는 것이 **의도된 설계**다. 이유는 해시 대조다 — 엔진은 이
파일을 고칠 때 `before_sha256`/`after_sha256`를 계산해 "계획한 바이트와 실제로 쓴
바이트가 같은가"를 검증한다(`corpus_io.py:1126`). 서식이 흔들리면 그 검증이 성립하지
않는다. 158개 이름 정비에서 이 대조가 안전장치였다.

그래서 되돌려도 **다음 ID 마이그레이션에서 다시 한 줄이 된다.** 원위치다.

부수 사실: 이 파일에 대한 엔진 쓰기는 `id_only_migration`에서만 허용된다
(`corpus_io.py:3474`). 즉 아주 드물게만 바뀐다.

`corpus_fingerprint`는 `store.all()`(객체)로만 계산되고 이 파일은 store 밖이라
(`store.py:54`) 서식을 바꿔도 코퍼스 지문에는 영향이 없다. 원래 적었던
"코퍼스 바이트가 달라진다"는 우려도 근거가 없었다.
...
## 9. 확인이 필요한 지점 — **승인받고 실행 완료** (2026-08-04)

작업 2(엔진 몸통 규칙)는 검증 의미를 바꾸는 변경이라 진행 전 승인이 필요했다.
승인받아 전부 실행했다.

대안은 데이터만 고치는 길이었는데, 실측해보니 좋지 않았다. `--2`는 인용문을 함수
...
| | |
|---|---|
| 엔진 몸통 규칙 | `ab27a9c` — pytest 1522(기존 1516 + 신규 6), ingest runtime 99 |
| `--3` 교정 | `symbol`·`title` → `calculateExpandedGameAreaRowCounts`, 짝 `evref` title 동일 |
| `audit` | **ok = true** — symbol mismatch 5 → 0, lint 0, 참조 3809 intact |
| 색인 | 7,884개 재구축 (mecab-ko + bge-m3) |
| 실코퍼스 checks / eval | 10개(건너뜀 0) / 15/15 |
| 최종 스냅샷 | `ad657ec5…` 11,132 파일, `.snapshots/2026-08-04/task17-final/`, verify PASS |
| Task 18 연결점 | `135ce054…`, 검증된 스냅샷 루트 바깥 |
| 백업 정리 | `.snapshots` 1.7GB → 102MB (중간 백업 14개 제거) |

**계획에 없던 발견 두 가지**를 `ROADMAP.md` 미뤄둔 작업에 기록했다.

- **7번** — Task 17 복구 번들 12개의 소속 판정, `run_migration.py`의 안전장치 네 가지,
  엔진 흡수 조건. BB2 커밋은 공유 방식 변경 결정으로 보류.
- **8번** — 옛 앵커 수정을 막는 읽기/쓰기 비대칭. 축약 SHA(전체의 86%)와 미머지 브랜치
  커밋 둘 다 읽기는 통과하고 쓰기는 거부한다. `--3`은
  `--expected-revision-ref side/ingame-resolution`으로 우회했다.

영수증: `task17-complete-2026-08-04.json`, `snapshot-cleanup-2026-08-04.json`,
`final-snapshot-create-2026-08-04.json`.

...
| 지식 초안(#50 구현) | `draft.py`, `cli.py`, `installer.py`, `templates/draft/**` | `test_draft.py`, `test_draft_cli.py`, `test_agent_skill_contract.py`, `test_installer.py`, snapshot 제외 표적 테스트 | 다섯 번째 스킬 설치, 사용자 수정 보존, 첫/두 번째 install | 실제 BB2 `sally-canoe-glossary-audit`를 다른 새 세션에서 발견·재개·갱신(#51) | 불필요 | 초안은 BrainStore·raw·index 입력이 아니므로 불필요 | 고정 Markdown template, 단일 writer, expected SHA·원자 교체, 첫 버전에서 정식화·close·receipt 제외 |

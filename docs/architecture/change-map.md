...
| 공통 어휘 기준(#48 구현)·대표어/동의어/별칭 회수(#49 미구현) | `templates/ingest/references/glossary-criteria.md`, 관련 설치 스킬; 후속 `router.py`, `surface.py` | #48은 `test_agent_skill_contract.py`, `test_ingest_skill_contract.py`, `test_installer.py`; #49는 `test_router.py`, `test_surface.py`와 회수 표적 테스트 | #48의 ingest·session-ingest·audit 조건부 pointer와 installer 첫/두 번째 install. brain-draft 연결은 #50에서 추가 | #48은 설치 산출물 합성 검증. 실제 BB2 전체 기계 현황 + 샐리 카누 의미 판정은 #51에서 자동 수정 없이 읽기 전용 보고부터 | #48은 검색 결과를 바꾸지 않아 불필요. #49는 실제 BB2 eval·표적 회상 필요 | #48은 색인 입력을 바꾸지 않아 불필요. #49가 surface/index 입력을 바꾸면 필요 | 실제 프로젝트 이름·독립 개념·근거, 코드 토큰 제외, term+synonyms+aliases 동등 회수, 이번 범위에서 GCR·공통 verification 제외 |

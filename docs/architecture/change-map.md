...
| capability registry | `capabilities.py`와 이를 소비하는 `schema.py`, `query/router/search`, `promote.py`, `mutation.py`, `graph.py` | `test_capabilities.py` + 실제로 바꾼 소비 모듈 테스트 | 설치 스킬의 권한 설명이 바뀌면 contract+installer | 정책표만 추가하면 불필요. 공개 허용·거부를 바꾸면 해당 `brain/checks` | 검색 lane이 바뀌면 필요 | registry 설명만 바뀌면 불필요. surface 대상이 바뀌면 필요 | 19종 exact coverage, registry와 분산 runtime의 단계적 전환, 기존 공개 동작 보존 |
| common candidate verification·promotion | `verification.py`, `verification_event_time_code.py`, `hash_utils.py`, `schema.py`, `promote.py`, `mutation.py`, `cli.py` | `test_verification.py`, `test_verification_event_time_code.py`, `test_verification_domain_profiles.py`, `test_mutation.py`, `test_cli.py` | public ingest/promote 안내가 바뀌면 runtime+installer | 실제 candidate/promote smoke와 `brain/checks`; 저장 객체를 바꾸면 lint/audit | 회수 status나 실제 corpus가 바뀌면 필요 | hash projection 코드만 바뀌고 corpus/index 입력이 같으면 불필요. 실제 promote 뒤 후속 search/eval 전 필요 | v1 exact envelope, live content/evidence/rules/execution, fixed reason order, repo context, target+single ReviewRecord 원자성, 실패 0-write |
| dedicated proof capture·derived | `dedicated_proof.py`, `dedicated_proof_capture.py`, `dedicated_proof_derived.py`, `mutation.py`, `corpus_io.py`, `ingest.py`, projection 경로 | `test_dedicated_proof.py`, `test_dedicated_proof_capture.py`, `test_dedicated_proof_derived.py`, `test_mutation.py`, `test_corpus_io.py`, `test_ingest.py`, `test_cli.py` | public Adapter·template 변경이면 runtime+installer 2회 | 실제 source/corpus에 의존하면 표적 `brain/checks`와 lint/audit | 저장·회수 결과가 바뀌면 필요 | proof/receipt 코드만 바뀌면 불필요. 실제 object action 뒤 후속 search/eval 전 필요 | proof는 객체가 아닌 manifest 증거, target action·live source·receipt 결속, common ReviewRecord 없음, source drift 0-write, journal rollback |
...
| session·session-ingest | `session.py`, `cli.py`, `templates/session-ingest/**`, 완료 결속을 추가하면 batch/receipt contract | `test_session.py`, `test_cli.py`, `test_corpus_io.py`, `test_architecture_docs.py`, session CLI와 skill contract, 설치 `test_batch_tools.py` | session-ingest/template/batch runtime이 바뀌면 installer 2회와 runtime unittest | 실제 transcript scan·marker smoke는 필요할 때 별도 | 불필요 | 불필요 | 현재 marker는 UUID·시각·note뿐인 코퍼스 밖 약한 표시. 미래 completion은 transcript·verification·durable batch/finalization receipt 결속과 legacy·재시도·동시성까지 별도 검증 |
...
내부 proof validator 테스트만으로 공개 적재 계약을 검증한 것으로 보지 않는다. public Adapter를
추가하거나 바꾸면 기본 profile을 비활성화하지 않은 CLI·ingest 회귀를 반드시 포함한다.

...
| 공통 어휘 기준·대표어/동의어/별칭(replacement 대상, 미구현) | 예정 `templates/ingest/references/glossary-criteria.md`; `router.py`, `surface.py`, 관련 설치 스킬 | `test_router.py`, `test_surface.py`, `test_agent_skill_contract.py`, `test_installer.py`와 어휘 정답·반례 표적 테스트 | reference pointer를 쓰는 ingest·session-ingest·audit·brain-draft 계약, installer 첫/두 번째 install | 실제 BB2 전체 기계 현황 + 샐리 카누 의미 판정. 자동 수정 없이 읽기 전용 보고부터 | 별칭 회수 결과가 바뀌므로 실제 BB2 eval·표적 회상 필요 | alias를 기존 indexed surface에 추가하지 않고 router/mapping 회수만 고치면 불필요. surface/index 입력이 바뀌면 필요 | 실제 프로젝트 이름·독립 개념·근거, 코드 토큰 제외, term+synonyms+aliases 동등 회수, 이번 범위에서 GCR·공통 verification 제외 |

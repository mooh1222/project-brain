...
| capability 정책 registry(내부) | `capabilities.py` | `test_capabilities.py` | 현재 설치 runtime 영향 없음 | 불필요 | 불필요 | 불필요 | 현재 19종 정책 설명·드리프트 검사 전용. schema·query·promote 등 runtime 분기를 대체하지 않음 |
| evidence plan·preparation(내부 미연결) | `evidence_plan.py`, `evidence_preparation.py`, `capabilities.py`, `foundation.py`, `snapshot.py` | `test_evidence_plan.py`, `test_evidence_preparation.py`, 관련 foundation·snapshot 표적 테스트 | 내부 계약만 바꾸면 불필요. public ingest·promote나 template에 연결하는 별도 승인 변경이면 runtime + installer 2회 | 내부 계약만이면 불필요. 실제 corpus 쓰기 경로에 연결할 때만 변경 축에 맞춰 추가 | 내부 계약만이면 불필요 | 내부 계약만이면 불필요 | 현재 production caller 없음. 직접 테스트 PASS와 public feature 활성화를 구분하며 #44~#46은 보류 |
...
| 공통 어휘 기준(#48 구현)·대표어/동의어/별칭 회수(#49 구현) | `templates/ingest/references/glossary-criteria.md`, 관련 설치 스킬, `router.py`, `surface.py` | #48은 `test_agent_skill_contract.py`, `test_ingest_skill_contract.py`, `test_installer.py`; #49는 `test_router.py`, `test_surface.py`와 회수 표적 테스트 | #48의 ingest·session-ingest·audit 조건부 pointer와 installer 첫/두 번째 install. brain-draft 연결은 #50에서 추가 | #48은 설치 산출물 합성 검증. #49는 실제 BB2 대표어·동의어·별칭 표적 query + `brain/checks`. 전체 어휘 기계 현황 + 샐리 카누 의미 판정은 #51에서 자동 수정 없이 읽기 전용 보고부터 | #48은 검색 결과를 바꾸지 않아 불필요. #49는 실제 BB2 eval·표적 회상 필요 | #48은 색인 입력을 바꾸지 않아 불필요. #49는 mapping surface에 aliases를 추가하고 extractor v4로 올렸으므로 실모델 rebuild 필요 | 실제 프로젝트 이름·독립 개념·근거, 코드 토큰 제외, term+synonyms+aliases 동등 회수, 이번 범위에서 GCR·공통 verification 제외 |
...
| 지식 초안(replacement 대상, 미구현) | 예정 draft 모듈·`cli.py`·`installer.py`·`templates/draft/**` | create/list/show/update/lint, expected SHA, 경로·symlink, 비색인 경계와 installer 계약 표적 테스트 | 다섯 번째 스킬 설치, 사용자 수정 보존, 첫/두 번째 install | 실제 BB2 `sally-canoe-glossary-audit`를 다른 새 세션에서 발견·재개·갱신 | 불필요 | 초안은 BrainStore·raw·index 입력이 아니므로 불필요 | 고정 Markdown template, 단일 writer, expected SHA·원자 교체, 첫 버전에서 정식화·close·receipt 제외 |

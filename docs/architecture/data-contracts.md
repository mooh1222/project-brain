...
| **DomainMapping** / `domain` | R `context_id`, `mapping_key`, `canonical_summary`, `meaning`, `boundary`, `glossary_term_ids`, `decision_record_ids`; reviewed면 evidence 1+. O `review_state`는 정해진 4개 boolean 키만 | `mapping.<ctx>.<key>`와 context/key 결속. mappings build, 직접 입력, bundle promote. 적재자가 의미·경계·refs를 쓰고 build가 ID/meta 및 decision inverse를 만든다 | context 1; glossary/decision 0+; O code locators/superseded mappings 0+, review record 1; evidence reviewed 1+ | 핵심 query 객체, graph rerank, projection, term promotion, supersession/decision lint. 색인은 참조 GlossaryTerm의 term/synonyms/aliases를 같은 이름 표면으로 이어받는다. reviewed↔candidate 후퇴 금지; superseded target lifecycle도 merged lint를 통과해야 한다 |

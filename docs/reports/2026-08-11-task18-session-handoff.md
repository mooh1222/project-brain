...
2. 다음 작업이 정해지면 현행 ROADMAP의 착수 조건과 현재 코드·Git 상태를 다시 대조한 뒤 새
   범위를 잡는다. 과거 Task 18 계획의 미체크 항목을 실행 목록으로 삼지 않는다.
3. 사용자 요청에 따라 이 세션에서 이 handoff 한 파일만 docs-only commit으로 남길 예정이다.
   이후 에이전트는 이 문서를 재커밋하거나 아래 제품 closure를 다시 만들거나 검증하지 않는다.
...
- `task18-closure.json` SHA-256은
  ...
  원래 사용자 dirt engine 15건·BB2 12건을 보존했고 양쪽 staged는 0/0이다. 엔진 2,077
...
- BB2의 수정된 `brain/checks/test_real_corpus.py`와 ignored
  `brain/raw/sources/petskill-kamehameha/spec-v1.1.md`는 현재 남아 있는 사용자 소유 파일이라
  자동 커밋하면 안 된다. recovery bundle 공유 방식은 ROADMAP 미뤄둔 작업 7번의 보류 결정이며,
  옛 검색 회귀는 현행 ROADMAP에 없다. 필요하면 현재 코드에서 다시 측정하고 새 범위를 승인받는다.
  결론은 **옛 명령을 그대로 실행하지 말고 잔여를 현재 상태에서 다시 판정한다**는 것이다.
...
- attempt-006 snapshot과 receipt는 ignored 로컬 증거다. 이번 세션에서는
  `/Users/al03040455/Downloads/codes/project-brain/docs/reports/2026-08-11-task18-session-handoff.md`
  한 파일만 docs-only commit할 예정이다. 테스트, migration, corpus 수정, push/PR은 하지 않았다.
...
없음. Task 18의 필수 후속은 없고, 다음 제품 우선순위만 사용자가 별도로 선택하면 된다.
...
- engine과 BB2 양쪽에 Task 18 밖의 기존 사용자 변경이 남아 있다. 원복·삭제·수정·스테이지하지
  않고 `git add -A`, `git add .`, `git commit -a`도 쓰지 않는다.

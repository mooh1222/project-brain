...
def resolve_git_checkout(path: Path) -> GitCheckoutState:
    """Resolve the actual Git toplevel and exact HEAD containing ``path``."""
    candidate = Path(path)
    if not candidate.is_absolute():
        raise ValueError("Git checkout path must be absolute")
    if "\x00" in str(candidate):
        raise _failure("not_git_repo", "Git checkout path contains a NUL byte")
    try:
        resolved = candidate.resolve(strict=True)
    except (OSError, ValueError) as exc:
        raise _failure(
            "not_git_repo",
            f"Git checkout path could not be resolved: {exc}",
        ) from exc
    start = resolved if resolved.is_dir() else resolved.parent
    top_result = _run_git(start, "rev-parse", "--show-toplevel")
    if top_result.returncode != 0:
        raise _failure(
            "not_git_repo",
            f"git rev-parse --show-toplevel failed: {_stderr(top_result)}",
    ...
    try:
        top_text = top_result.stdout.decode(
            "utf-8",
            errors="strict",
        ).strip()
    except UnicodeDecodeError as exc:
        raise _failure(
            "not_git_repo",
            f"Git toplevel is not UTF-8: {exc}",
        ) from exc
    if not top_text:
        raise _failure("not_git_repo", "git returned an empty checkout root")
    root = Path(top_text).resolve()
    if root != resolved and root not in resolved.parents:
        raise _failure(
            "repo_identity_mismatch",
            f"path {resolved} is outside Git toplevel {root}",
        )
    head_result = _run_git(root, "rev-parse", "--verify", "HEAD^{commit}")
    if head_result.returncode != 0:
        raise _failure(
            "commit_missing",
            f"checkout HEAD is unavailable: {_stderr(head_result)}",
        )
    try:
        head_sha = head_result.stdout.decode(
            "ascii",
            errors="strict",
        ).strip()
    except UnicodeDecodeError as exc:
        raise _failure(
            "commit_missing",
            f"checkout HEAD is not ASCII: {exc}",
        ) from exc
    if _GIT_SHA.fullmatch(head_sha) is None:
        raise _failure(
            "commit_missing",
            "checkout HEAD is not an exact lowercase Git SHA",
        )
    root_stat = root.stat()

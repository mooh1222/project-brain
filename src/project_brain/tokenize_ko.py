...
2. 한국어 형태소 = ★kiwipiepy 단일 백엔드★(#79). 형태소 조각에 더해 같은 어절 안에서
   연속한 명사류 조각(뒤에 바로 붙은 파생 접미사 포함)을 이어 붙인 결합형 토큰과, 동사
   어간+명사형 전성어미의 어절 표면(알림·만들기)도 보존한다 — 심볼에서 camelCase 조각과 원형을 같이 남기는 것과 같은 규칙이다. 폴백 사다리는 없다: 설치 환경마다 다른
...
# 버전 4 = 명사류 연속 뒤에 바로 붙은 파생 접미사(XSN)를 같은 결합형에 포함(시간제·스테이지별·
# 이벤트용·활성화, #82). 전부 산출 토큰이 늘어난 규칙 변경이라 change map "한국어 tokenizer" 행의
# 실모델 rebuild가 발동한다.
TOKENIZER_RULES_VERSION = 4
...
# 명사 파생 접미사(-제·-별·-용·-화·-성·-당·-형·-자·-들). kiwi는 "시간제"를 시간(NNG)+제(XSN)로
# 읽으므로 접미사에서 연속이 끊기면 "시간제" 토큰이 어디에도 생기지 않는다(pb#81 실측). 접미사는
# 이미 시작된 명사류 연속을 ★확장만★ 한다 — 연속을 시작하지는 못한다(대명사 뒤 "들"처럼 앞에 명사류
# 조각이 없으면 결합형 없음). 규칙 버전 4(#82).
_COMPOUND_SUFFIX_TAG = "XSN"
...
def _eojeol_surfaces(tokens, text: str) -> list[str]:
    """한 어절의 형태소 토큰들 → 조각 표면들 + 결합형들 + 명사형 표면들 (#79, 규칙 버전 4).

    ★조각을 지우지 않고 표면을 더한다★ — "인게임"으로 물어도 "게임"으로 물어도 같은 문서가
    잡혀야 하기 때문이다.
    - 결합형: 연속한 명사류 조각(_COMPOUND_TAGS)이 2개 이상이고 그중 한글 조각이 하나 이상이면
      이어 붙여 소문자로 추가한다(인게임·럭키박스·3단계·2차·ui버튼). 명사류 뒤에 바로 붙은 파생
      접미사(_COMPOUND_SUFFIX_TAG)는 연속에 포함된다(시간제·스테이지별·활성화). 조사·어미·동사
      조각이 나오면 연속이 끊긴다.
    ...
    for token in tokens:
        ...
        if token.tag in _COMPOUND_TAGS or (
            token.tag == _COMPOUND_SUFFIX_TAG and run
        ):
            run.append(form)
...
def parse_tokenizer_signature(value: str) -> tuple[str, int]:
    ...
    만들어진 색인(이름만 기록)은 버전 1 규칙으로 만들어진 것이기 때문이다. 현재 규칙
    (TOKENIZER_RULES_VERSION)은 그보다 높으므로 그런 색인은 호출부에서 불일치로 거부되고
    rebuild가 필요하다.

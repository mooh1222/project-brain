...
2. kind별 후보 추출: DecisionRecord·GlossaryTerm·DomainMapping·TemporalFact(값 변경이면
   ...
   에이전트 요약을 raw 원문에 섞지 않는다. 개인 메모리는 적재 안 하고 표시만 한다.
...
4. ingest → `{{PROJECT}}-brain-ingest/references/completeness-checklist.md` 최종화. canonical receipt가
   exact이고 실패가 0이며 `finalization.ok=true`일 때만 `project-brain session mark-processed <uuid>`를
   실행한다. 적재나 최종화가 실패하면 unprocessed 상태를 유지하고 재개 정보를 남긴다.

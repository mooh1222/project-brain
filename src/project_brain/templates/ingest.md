...
8. **코드 앵커는 기본 브랜치 기준 `commit_sha`.** 작업 브랜치가 아니라 기본
   브랜치의 심볼을 박는다(줄번호는 엔진이 저장하지 않는다 — 의미 앵커는 심볼+`commit_sha`,
   코드 변경 감지(stale-check)는 path+`commit_sha`). `mapping_key`는 논리적 의미, `code_locator`가 코드

...
`project-brain index rebuild`가 필요하다. 명사류 조각 뒤에 바로 붙은 파생 접미사(-제·-별·-용·
-화·-성 등)를 같은 어절 결합형에 포함해 `시간제`·`스테이지별`·`이벤트용`·`활성화`가 토큰으로
남는다(kiwi가 `시간제`를 시간+제로 읽어 그 토큰이 어디에도 없던 문제, #82). 접미사는 연속을
시작하지 못하고 확장만 한다. 조각은 그대로 두고 표면만 더하므로 기존 조각 질의는 그대로 맞는다.
스킬 파일 변경은 없다.

## 2026-09-04 — 토크나이저 규칙 버전 3: 숫자·외국어 결합형과 명사형 전성어미 표면

토큰 산출 규칙이 2에서 3으로 올라가 기존 색인은 검색 진입에서 `StaleIndexError`가 나고
`project-brain index rebuild`가 필요하다. (1) 어절 안 결합형에 숫자·외국어 조각을 허용한다 —
한글 조각이 하나 이상이면 `3단계`·`2차`·`ui버튼`(소문자)도 토큰으로 남는다. (2) kiwi가 `알림`을
문맥에 따라 명사 또는 동사 어간+명사형 어미로 달리 읽는 중의성을 흡수하기 위해, 어간부터
명사형 전성어미(-ㅁ/-기)까지의 어절 표면(`알림`·`만들기`·`보여주기`)을 토큰으로 함께 보존한다.
조각은 그대로 두고 표면만 더하므로 기존 조각 질의는 그대로 맞는다. 스킬 파일 변경은 없다.

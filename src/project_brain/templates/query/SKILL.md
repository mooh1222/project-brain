...
3. **적중 객체 원문 열람** — 검색 surface는 잘린다. 핵심 매핑은 `project-brain show
   <object_id>`로 객체 본문(meaning·boundary·caveats)과 연결된 이웃(코드 위치·용어·결정·
   근거)을 종류·제목과 함께 펼쳐 읽는다(JSON 파일 직접 열기 대체).

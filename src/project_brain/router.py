        ...
        for intent in classified.intents:
            if intent == "why_changed":
                ...
                if locator_aggregate is not None:
                    ...
                    # 검수완료 채널도 후보 채널(위 candidate_locators)과 같은 모양으로
                    # path·symbol을 동반한다 — bare id만 내면 답을 받는 쪽이 위치를 구분할
                    # 수 없다(같은 함수 안의 비대칭). 색인이 없거나 낡은 경로는 위에서
                    # 집계로 대체되므로 여기 오지 않는다.
                    section["locators"] = [
                        {"id": locator["id"],
                         "path": locator.get("path"),
                ...
                if candidate_details:
                    ...
                    if not matched_mappings:
                        clarification_needed = True
                code_changed_mappings = [
                    m for m in mapping_details
                    if m.get("stale_advisory", {}).get("code_changed")
                ]
                if code_changed_mappings:

...
@pytest.mark.parametrize("future_directory", ["objects/future", "objects/domain"])
def test_version_2_restore_rejects_later_kind_before_mutation(
    ...
    request, paths = _snapshot_fixture(tmp_path)
    result = create_snapshot(request)
    future_path = request.brain_root / future_directory / "future.json"
    _write(
        future_path,
        json.dumps({"id": "future", "kind": "FutureKind"}).encode("utf-8"),
    )
    before = {
    ...
    assert caught.value.code == "restore_target_kind_unsupported"
    assert "FutureKind" in caught.value.detail
    assert {
        path.relative_to(request.brain_root).as_posix(): path.read_bytes()
        for path in request.brain_root.rglob("*")
        if path.is_file()
